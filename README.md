...
源任务通常必须空闲且无在途工具。若源客户端漏报 `Stop`，持有目标目录当前轮次租约的合并会话可以在源目录没有在途工具和交接等待、提交已固定且工作区干净时封存源任务；封存先关闭 Hook 闸门，旧源会话随后发出的工具会被拒绝。没有有效目标租约时仍不允许封存活动源会话。目标目录不能有冲突中的 Git 操作或无关未提交改动。清理前再次验证提交已合入目标，并检查未跟踪及忽略文件；不会强制删除 worktree。多个任务逐个合并并验证。合并冲突或测试失败时保留现场与任务记录。
