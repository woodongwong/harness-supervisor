...
When the integration supplies a `task-state` update command, persist the concrete goal, constraints, decisions, unfinished work, and verification outcomes for substantive development. Do this as those facts change and before ending a development turn; the user does not need to request a handoff or issue bookkeeping commands. Save known pending work early enough to survive interruption. Ordinary questions need no state update unless they change the task's decisions or constraints. Legacy handoffs without an update entry do not support this mechanism; preserve explicit pending work in the final response without inventing a lease or claiming persistent storage.
