...
export class ZCodeAdapter {
  constructor({ binary = process.env.ZCODE_BIN ?? "zcode", argsTemplate = null, resumeArgsTemplate = null, spawnImpl = nodeSpawn } = {}) {
    this.name = "zcode";
    this.binary = binary;
    this.argsTemplate = argsTemplate ?? parseJsonArrayEnv("ZCODE_RELAY_ARGS_JSON", [], "ZCODE_SUPERVISOR_ARGS_JSON");
    this.resumeArgsTemplate = resumeArgsTemplate
      ?? ((process.env.ZCODE_RELAY_RESUME_ARGS_JSON ?? process.env.ZCODE_SUPERVISOR_RESUME_ARGS_JSON)
        ? parseJsonArrayEnv("ZCODE_RELAY_RESUME_ARGS_JSON", [], "ZCODE_SUPERVISOR_RESUME_ARGS_JSON")
        : null);
  ...
  async run({ task, prompt, resumeSessionId = null, onEvent = async () => {}, taskDir, contextPath, externalEventsPath, signal }) {
    ...
    if (resumeSessionId && !this.resumeArgsTemplate) {
      throw new Error("ZCode resume is not configured. Set ZCODE_RELAY_RESUME_ARGS_JSON if your launcher exposes resume.");
    }
    ...
    const env = {
      ...process.env,
      HARNESS_RELAY_TASK_ID: task.id,
      HARNESS_RELAY_TASK_DIR: taskDir,
      HARNESS_RELAY_CONTEXT_FILE: contextPath,
      HARNESS_RELAY_EVENT_SINK: externalEventsPath,
      // Older installed bridge versions still consume the legacy names.
