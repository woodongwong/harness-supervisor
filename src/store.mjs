...
import { relayHome } from "../zcode-plugin/hooks/runtime-config.mjs";

export class TaskStore {
  constructor(root = relayHome()) {
  ...
  async create({ goal, cwd, primary, fallback, id = newTaskId() }) {
  ...
  async writeContext(id, text) {
    const target = this.contextPath(id);
    const tmp = `${target}.${process.pid}.${crypto.randomUUID()}.tmp`;
    await fs.writeFile(tmp, String(text), { mode: 0o600 });
    await fs.rename(tmp, target);
  }
  ...
  async withTaskLock(id, fn) {
    ...
    const { autoStatus } = await import("./auto-handoff.mjs");
    if (await autoStatus(this.root, cwd)) throw new Error("此项目已启用自动交接，请在 Codex / ZCode 原生会话中继续；不能混用旧 run/takeover/bind 命令。");
